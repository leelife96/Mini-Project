...
public class OmokBoard {
	    ...
	    for (int row = 0; row < height; row++) {
	        for (int col = 0; col < width; col++) {
	            if (buf[row][col] == stone) {    // 현재 검사 중인 셀이 지정한 돌과 일치하는지 확인
	                ...
	                if (checkDirection(row, col, 0, 1, stone, 1) ||		// 가로 방향 승리 확인
	...
	private boolean checkDirection(int row, int col, int deltaRow, int deltaCol, char stone, int count) {    // 특정 방향으로 승리 여부를 확인하기 위한 메소드
	    if (count == 5) {  		// 현재 방향에서 돌이 5개 연속으로 나타난 경우 승리 조건을 만족하고 true 반환 
	        return true;
	    }
	    int newRow = row + deltaRow;	// 현재 위치에서 주어진 방향으로 한 칸 이동한 새로운 위치를 계산	
